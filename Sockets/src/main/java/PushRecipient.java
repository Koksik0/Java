...
public class PushRecipient extends Recipient {
  ...
  void validateRecipient() throws SenderException {
    ...
    boolean isCorrectRecipientAddress = Pattern.compile(regexPattern)
            .matcher(recipientAddress)
            .matches();
     if(!isCorrectRecipientAddress)
       throw new SenderException("Invalid recipient address");
     if(recipientAddress.length()!=32)
       throw new SenderException("Recipient address is too short or too long. The recipient address should be 32 characters");
  }
  @Override
  String anonymize() {
